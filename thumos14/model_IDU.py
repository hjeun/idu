...
class Model(object):
    ...
    def build_network(self):
        ...
        with tf.variable_scope('Output'):
            ...
            logits_last = tf.layers.dense(logits, 21, activation=None)
            logits_last = tf.reshape(logits_last, [-1, 16, 21])
    ...
    def embedding(self, input):
        ...
        embedding_last = tf.layers.dense(embedding, 21, activation=None)
        embedding = tf.reshape(embedding, [-1, 16, 512])
        embedding_last = tf.reshape(embedding_last, [-1, 16, 21])
