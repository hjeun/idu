...
class RNN(Layer):
  ...
  def __call__(self, inputs, initial_state=None, constants=None, **kwargs):
    self.cell.input_t_zero = inputs[:, -1, :]
  ...
  def call(self,
    ...
    if constants:
      ...
      def step(inputs, states):
        states = states[0] if len(states) == 1 and is_tf_rnn_cell else states
        output, new_states, gates = self.cell.call(inputs, states, **kwargs)
        if not nest.is_sequence(new_states):
          new_states = [new_states]
        return [output, gates], new_states
...
@tf_export('keras.layers.IDUCell')
class IDUCell(Layer):
  """Cell class for the GRU layer.
  ...
  def __init__(self,
               ...
               reset_after=False,
               **kwargs):
    super(IDUCell, self).__init__(**kwargs)
    self.units = units
    self.activation = activations.get(activation)
    self.recurrent_activation = activations.get(recurrent_activation)
    self.activation_sigmoid = activations.get('sigmoid')
    self.activation_softmax = activations.get('softmax')
    self.use_bias = use_bias
  ...
  def build(self, input_shape):
    input_dim = input_shape[-1]
    self.kernel_512_512 = self.add_weight(
      shape=(512, 512, 3 * 2),
      name='kernel_512_512',
      initializer=self.kernel_initializer,
      regularizer=self.kernel_regularizer,
      constraint=self.kernel_constraint)
    self.kernel_2_512_512 = self.add_weight(
        shape=(2, 512, 512, 3),
        name='kernel_2_512_512',
        initializer=self.recurrent_initializer,
        regularizer=self.recurrent_regularizer,
        constraint=self.recurrent_constraint)
  ...
  def fc_n_add(self, a, a_kernel, b, b_kernel, act):
    a_ = K.dot(a, a_kernel)
    b_ = K.dot(b, b_kernel)
    c = a_ + b_
    if act == 'tanh':
      ...
      c = self.activation_sigmoid(c)
    elif act == 'hsigm':
      c = self.recurrent_activation(c)
    return c

  def fc_n_sub(self, a, a_kernel, b, b_kernel, act):
    a_ = K.dot(a, a_kernel)
    b_ = K.dot(b, b_kernel)
    c = a_ - b_
    if act == 'tanh':
  ...
  def concat_n_conv(self, a, b, c_kernel, act):
    a_ = K.expand_dims(a, axis=1)
    b_ = K.expand_dims(b, axis=1)
    c = K.concatenate((a_, b_), axis=1)
    c = K.conv1d(c, c_kernel, padding='valid')
    c = K.squeeze(c, axis=1)
    if act == 'tanh':
      c = self.activation(c)
    elif act == 'sigm':
      c = self.activation_sigmoid(c)
  ...
  def type1(self, inputs, inputs0, h_tm1):
    inputs_z, inputs_r, inputs_h = inputs
    inputs0_z, inputs0_r, inputs0_h = inputs0
    h_r = h_tm1

    inputs0_r_kernel = self.kernel_512_512[:, :, 1]
    inputs_z_kernel = self.kernel_512_512[:, :, 2]
    inputs0_z_kernel = self.kernel_512_512[:, :, 3]
    inputs_h_kernel = self.kernel_512_512[:, :, 4]
    h_h_kernel = self.kernel_512_512[:, :, 5]
    h_r_kernel = self.kernel_512_512[:, :, 0]

    r_t = self.fc_n_add(h_r, h_r_kernel, inputs0_r, inputs0_r_kernel, 'hsigm')
    z_t = self.fc_n_add(inputs_z, inputs_z_kernel, inputs0_z, inputs0_z_kernel, 'hsigm')
    h_h = r_t * h_tm1
    h_t = self.fc_n_add(inputs_h, inputs_h_kernel, h_h, h_h_kernel, 'tanh')

    h_t = (1 - z_t) * h_tm1 + (h_t * z_t)
    return h_t, [r_t, h_t]

  def type2(self, inputs, inputs0, h_tm1):
    inputs_z, inputs_r, inputs_h = inputs
    inputs0_z, inputs0_r, inputs0_h = inputs0
    h_r = h_tm1

    inputs_r_kernel = self.kernel_512_512[:, :, 0]
    inputs0_r_kernel = self.kernel_512_512[:, :, 1]
    inputs_z_kernel = self.kernel_512_512[:, :, 2]
    inputs0_z_kernel = self.kernel_512_512[:, :, 3]
    h_t_kernel = self.kernel_2_512_512[:, :, :, 0]
    h_r_kernel = self.kernel_512_512[:, :, 4]

    r_t = self.fc_n_add(h_r, h_r_kernel, inputs0_r, inputs0_r_kernel, 'hsigm')
    z_t = self.fc_n_add(inputs_z, inputs_z_kernel, inputs0_z, inputs0_z_kernel, 'hsigm')
    h_h = r_t * h_tm1
    h_t = self.concat_n_conv(inputs_h, h_h, h_t_kernel, 'tanh')

    h_t = (1 - z_t) * h_tm1 + (h_t * z_t)
    # h_t = (h_t * z_t)
    return h_t

  def type3(self, inputs, inputs0, h_tm1):
    inputs_z, inputs_r, inputs_h = inputs
    inputs0_z, inputs0_r, inputs0_h = inputs0
    h_r = h_tm1

    r_t_kernel = self.kernel_2_512_512[:, :, :, 0]
    z_t_kernel = self.kernel_2_512_512[:, :, :, 1]
    input_h_kernel = self.kernel_512_512[:, :, 0]
    h_h_kernel = self.kernel_512_512[:, :, 1]

    r_t = self.concat_n_conv(h_r, inputs0_r, r_t_kernel, 'hsigm')
    z_t = self.concat_n_conv(inputs_z, inputs0_z, z_t_kernel, 'hsigm')
    h_h = r_t * h_tm1
    h_t = self.fc_n_add(inputs_h, input_h_kernel, h_h, h_h_kernel, 'tanh')
  ...
  def call(self, inputs, states, training=None):
    ...
    if 0. < self.dropout < 1.:
      ...
      inputs_r = inputs * dp_mask[1]
      inputs_h = inputs * dp_mask[2]
      inputs0_z = self.input_t_zero * dp_mask[3]
      inputs0_r = self.input_t_zero * dp_mask[4]
      inputs0_h = self.input_t_zero * dp_mask[5]
    else:
      inputs_z = inputs
      inputs_r = inputs
      inputs_h = inputs
      inputs0_z = self.input_t_zero
      inputs0_r = self.input_t_zero
      inputs0_h = self.input_t_zero

    h, gates = self.type1([inputs_z, inputs_r, inputs_h], [inputs0_z, inputs0_r, inputs0_h], h_tm1)
    # h = self.type2([inputs_z, inputs_r, inputs_h], [inputs0_z, inputs0_r, inputs0_h], h_tm1)
    # h = self.type3([inputs_z, inputs_r, inputs_h], [inputs0_z, inputs0_r, inputs0_h], h_tm1)
    return h, [h], gates
  ...
  def get_config(self):
    ...
    base_config = super(IDUCell, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))
...
class IDU(RNN):
  ...
  def get_config(self):
    ...
    base_config = super(IDU, self).get_config()
    del base_config['cell']
    return dict(list(base_config.items()) + list(config.items()))
